...
const HouseholdCostComponent = ({ householdData }) => {
    const sortedData = [...householdData].sort((a, b) => b.totalValue - a.totalValue);
    const leadingMember = sortedData[0];
    const secondMember = sortedData[1];

    const valueDifference = secondMember ? leadingMember.totalValue - secondMember.totalValue : 0;

    const appreciationMessage = secondMember
        ? `${leadingMember.username} has worked for ${valueDifference.toLocaleString()} nok more than ${secondMember.username}. Remember to show them you appreciate them!`
...
HouseholdCostComponent.propTypes = {
    householdData: PropTypes.arrayOf(PropTypes.shape({
